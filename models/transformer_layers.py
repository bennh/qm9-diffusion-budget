...
class EdgeGateTransLayer(MessagePassing):
    ...
    def forward(self, x: OptTensor,
        ...
        x_feat = x
        q = self.lin_query(x_feat).view(-1, H, C)
        k = self.lin_key(x_feat).view(-1, H, C)
        v = self.lin_value(x_feat).view(-1, H, C)

        # propagate_type: (q: Tensor, k: Tensor, v: Tensor, edge_attr: OptTensor)
    ...
    def message(self, q_i, k_j, v_j, edge_attr, index, ptr, size_i):
        ...
        alpha = (q_i * k_j * edge_attn).sum(dim=-1) / math.sqrt(self.out_channels)
