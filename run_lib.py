...
def mol_sde_train(config, workdir):
    ...
    if config.training.sde.lower() == 'vpsde':
        atom_sde = sde_lib.VPSDE(
            beta_min=config.model.node_beta_min,
            beta_max=config.model.node_beta_max,
            N=config.model.num_scales
        )
        bond_sde = sde_lib.VPSDE(
            beta_min=config.model.edge_beta_min,
            beta_max=config.model.edge_beta_max,
            N=config.model.num_scales
        )
        sampling_eps = 1e-3
    else:
    ...
    if config.training.snapshot_sampling:
        sampling_atom_shape = (
            ...
            config.data.atom_channels
        )
        sampling_bond_shape = (
            config.training.eval_batch_size,
            config.data.bond_channels,
            config.data.max_node,
            config.data.max_node
        )
        sampling_fn = sampling.get_mol_sampling_fn(
            config,
            atom_sde,
            bond_sde,
            sampling_atom_shape,
    ...
    for step in range(initial_step, num_train_steps + 1):
        ...
        if step != 0 and step % config.training.snapshot_freq == 0 or step == num_train_steps:
            ...
            if config.training.snapshot_sampling:
                ...
                sample_list, valid_wd = tensor2mol(
                    atom_sample,
                    bond_sample,
                    sample_nodes,
                    config.data.atom_list,
                ...
                logging.info(
                    "step: %d, n_mol: %d, validity rate wd check: %.4f, fcd_val: %.4f, fcd_test: %.4f" %
...
def mol_sde_evaluate(config, workdir, eval_folder="eval"):
    ...
    if config.training.sde.lower() == 'vpsde':
        ...
        atom_sde = sde_lib.subVPSDE(
            beta_min=config.model.node_beta_min,
            beta_max=config.model.node_beta_max,
            N=config.model.num_scales
        )
        bond_sde = sde_lib.subVPSDE(
            beta_min=config.model.edge_beta_min,
            beta_max=config.model.edge_beta_nax,
            N=config.model.num_scales
        )
        sampling_eps = 1e-3
    else:
    ...
    for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
        ...
        if config.eval.enable_sampling:
            ...
            logging.info(
                'sampling -- ckpt: {}, validity w/o correction: {:.6f}'.format(
                    ckpt, np.sum(all_valid_wd) / len(all_valid_wd)
                )
            )
            ...
            for metric in ['valid', f'unique@{len(smile_list)}', 'FCD/Test', 'Novelty']:
                if metric in scores:
                    ...
                    logging.info(f'sampling -- ckpt: {ckpt}, {metric}: N/A')
